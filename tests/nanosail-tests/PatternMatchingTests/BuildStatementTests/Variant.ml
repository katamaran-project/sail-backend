    ...
    let tc =
      ...
      let* pattern_tree =
        ...
        let* pattern_tree = adorn
            pattern_tree
            [
              (VariantCase (mkid "A1", Binder { identifier = mkid "x"; wildcard = false }))
            ]
            a1_statement
        in
        TC.return pattern_tree
...
let test_build_match_for_variant_single_binary_constructor =
  let test _ =
    let tc =
      let* variant_type =
        TC.define_variant "A" [("A1", [Int None; Int None])]
      in
      let a1_statement : Ast.Statement.t =
        ReadRegister (mkid "r1")
      in
