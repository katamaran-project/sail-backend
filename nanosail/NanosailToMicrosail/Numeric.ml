...
module rec Expression : sig
  ...
  let pp (numeric_expression : Ast.Numeric.Expression.t) =
    let rec pp level (numexp : Ast.Numeric.Expression.t) =
      ...
      | Minus (x, y) -> PP.annotate [%here] @@ parens_if 0 @@ PP.pp_binary_operation Coq.Operator.subtraction    [pp 0 x; pp 1 y]
      ...
      | PowerOf2 x   -> PP.annotate [%here] @@ parens_if 3 @@ PP.(horizontal [ PP.string "2^"; pp 4 x])
