...
(*
   Local Obligation Tactic :=
     identifier.
*)
let pp_local_obligation_tactic (tactic : PP.t) : PP.t =
  let lines_of_code = [
...
let pp_derive_eqdec_for (pp_type : PP.t) : PP.t =
  PP.annotate [%here] begin
    pp_derive (PP.string "EqDec") pp_type
  end
...
let pp_derive_no_confusion_for (pp_type : PP.t) : PP.t =
  PP.annotate [%here] begin
    pp_derive (PP.string "NoConfusion") pp_type
  end
...
*)
let pp_canonical (pp_identifier : PP.t) : PP.t =
  PP.annotate [%here] begin
    pp_sentence @@ PP.(separate_horizontally ~separator:space [ PP.string "Canonical"; pp_identifier ])
  end
