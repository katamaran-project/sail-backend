...
module Make (Custom : CustomValue) = struct
  ...
  module ValueFactory : ActualParser.ValueFactory = struct
    type t = Value.t

    let mk_symbol  = Value.Mk.symbol
    let mk_string  = Value.Mk.string
    let mk_integer = Value.Mk.integer
    let mk_bool    = Value.Mk.bool
    let mk_list    = Value.list_to_cons
  end

  module Parser = ActualParser.Make(ValueFactory)
end
